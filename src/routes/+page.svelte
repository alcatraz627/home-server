<script lang="ts">
	...
	const memColor = $derived(
		system.memUsedPercent >= 90 ? 'var(--danger)' :
		system.memUsedPercent >= 70 ? 'var(--warning)' : 'var(--success)'
	);
...
<div class="stats-row">
	...
	<div class="stat-card">
		...
		<div class="stat-bar">
			<div class="stat-fill" style="width: {Math.min(100, (system.loadAvg / system.cpuCount) * 100)}%; background: {loadColor()}"></div>
		</div>
	...
	{#each dashboard.disk as d}
		<div class="stat-card">
			...
			<div class="stat-value" style="color: {parseInt(d.usePercent) > 90 ? 'var(--danger)' : parseInt(d.usePercent) > 70 ? 'var(--warning)' : 'var(--success)'}">{d.usePercent}</div>
			<div class="stat-bar">
				<div class="stat-fill" style="width: {d.usePercent}; background: {parseInt(d.usePercent) > 90 ? 'var(--danger)' : parseInt(d.usePercent) > 70 ? 'var(--warning)' : 'var(--success)'}"></div>
			</div>
		...
		<div class="status-body">
			...
			<div class="status-metrics">
				...
				{#if dashboard.backups.lastRun}
					<span class="metric" class:success={dashboard.backups.lastRun.status === 'success'} class:failed={dashboard.backups.lastRun.status === 'failed'}>
...
<style>
	...
	h3 { font-size: 1rem; }
	.section-title { margin: 20px 0 12px; color: var(--text-muted); font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em; }
	...
	.status-body h3 { font-size: 0.9rem; margin-bottom: 6px; }

	.status-metrics { display: flex; flex-wrap: wrap; gap: 4px 10px; }
	.metric { font-size: 0.75rem; color: var(--text-secondary); }
	.metric.muted { color: var(--text-faint); }
	.metric.running { color: var(--warning); }
	.metric.failed { color: var(--danger); }

<script lang="ts">
  ...
  function handleCaptureKeydown(e: KeyboardEvent) {
    ...
    // Escape always cancels — never rebindable
    if (e.key === 'Escape') {
      cancelCapture();
      return;
    }
    ...
    // e.key already gives '?' for Shift+/ — use directly
    capturedKey = e.key;
    setCustomKey(capturingId, e.key);
    reload();
    // Brief flash so the user sees the captured key before the modal closes
...
<svelte:head>
  <title>Keyboard Shortcuts | Home Server</title>
</svelte:head>
...
<div class="shortcuts-page">
  ...
  <p class="page-desc">View and customize keyboard shortcuts. Click a key badge to rebind it.</p>

<script lang="ts">
  ...
  let centerImageSrc = $state<string | null>(null);
  let centerImageEl = $state<HTMLImageElement | null>(null);
  let centerImageSize = $state(20); // percentage of QR size
  let centerImageFileInput = $state<HTMLInputElement | null>(null);

  // When a center image is added, auto-escalate EC to H for best scan reliability
  let preImageEcLevel = $state<'L' | 'M' | 'Q' | 'H' | null>(null);
  ...
  function handleCenterImage(e: Event) {
    ...
    reader.onload = () => {
      centerImageSrc = reader.result as string;
      const img = new Image();
      img.onload = () => {
        centerImageEl = img;
        // Auto-escalate EC level for scan reliability
        if (errorLevel !== 'H') {
          preImageEcLevel = errorLevel;
          errorLevel = 'H';
          toast.success('Error correction raised to High for center image');
        }
      };
  ...
  function removeCenterImage() {
    centerImageSrc = null;
    centerImageEl = null;
    if (centerImageFileInput) centerImageFileInput.value = '';
    // Restore previous EC level
    if (preImageEcLevel) {
      errorLevel = preImageEcLevel;
      preImageEcLevel = null;
    }
  }
  ...
  function renderQR() {
    ...
    if (centerImageEl) {
      const imgSize = Math.round(canvasSize * (centerImageSize / 100));
      const padding = Math.round(imgSize * 0.1);
      const bgSize = imgSize + padding * 2;
      const x = Math.round((canvasSize - bgSize) / 2);
      const y = Math.round((canvasSize - bgSize) / 2);

      // White background with rounded corners
      const radius = Math.round(bgSize * 0.08);
      ctx.fillStyle = '#ffffff';
      ctx.beginPath();
      ...
      // Draw image centered in the background area
      ctx.drawImage(centerImageEl, x + padding, y + padding, imgSize, imgSize);
    }
  }
  ...
  <div class="layout">
    <div class="controls card">
      ...
      <div class="center-image-section">
        <span class="field-label">Center Image</span>
        {#if centerImageSrc}
          <div class="center-image-preview">
            <img src={centerImageSrc} alt="Center logo" class="center-thumb" />
            <div class="center-image-controls">
              <label class="size-label">
                <span>Size: {centerImageSize}%</span>
                <input type="range" min="10" max="30" step="1" bind:value={centerImageSize} />
              </label>
              <button class="btn-remove-image" onclick={removeCenterImage}>Remove</button>
            </div>
          </div>
        {:else}
          <button class="btn-add-image" onclick={() => centerImageFileInput?.click()}> Add logo / image </button>
          <input
            bind:this={centerImageFileInput}
            type="file"
            accept="image/*"

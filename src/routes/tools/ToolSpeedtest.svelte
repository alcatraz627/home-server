<script lang="ts">
  ...
  function loadHistory() {
    try {
      const raw = localStorage.getItem(SK_SPEEDTEST_HISTORY);
  ...
  function saveHistory() {
    try {
      localStorage.setItem(SK_SPEEDTEST_HISTORY, JSON.stringify(history.slice(0, 20)));
    } catch {
      // ignore
    }
  }
  ...
  async function runExternalTest() {
    ...
    try {
      ...
      toast.error(getErrorMessage(e, 'External test failed'));
      phase = 'idle';
    } finally {
      running = false;
    }

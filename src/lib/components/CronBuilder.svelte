<script lang="ts">
	...
	function parseInitialValue(expr: string | null) {
		...
		if (min !== '*' && !min.includes('/') && hour !== '*' && !hour.includes('/') &&
			dom === '*' && dow === '*') {
			frequency = 'daily';
		...
		switch (frequency) {
			...
			case 'hourly':
				return hourlyMinute === 0
					...
					: `Every hour at ${hourlyMinute} minutes past`;
			case 'daily':
	...
	let nextRuns = $derived(
		enabled ? getNextRuns(expression, 3) : []
	);
				...
				<div class="cb-field">
					...
					<div class="cb-chip-group">
						{#each DAYS_OF_WEEK as day}
							<button
								class="cb-chip"
								class:active={weeklyDay === day.value}

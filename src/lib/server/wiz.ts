...
function sendUdp(ip: string, message: object, timeout = 2000): Promise<any> {
	return new Promise((resolve) => {
		...
		socket.send(buf, 0, buf.length, WIZ_PORT, ip, (err) => {
			if (err && !resolved) { resolved = true; socket.close(); resolve(null); }
		});

		setTimeout(() => {
			if (!resolved) { resolved = true; socket.close(); resolve(null); }
		}, timeout);
	});
}
...
export const WIZ_SCENES: Record<number, string> = {
	...
	5: 'Fireplace', 6: 'Cozy', 7: 'Forest', 8: 'Pastel Colors',
	9: 'Wake Up', 10: 'Bedtime', 11: 'Warm White', 12: 'Daylight',
	13: 'Cool White', 14: 'Night Light', 15: 'Focus', 16: 'Relax',
	17: 'True Colors', 18: 'TV Time', 19: 'Plant Growth', 20: 'Spring',
	21: 'Summer', 22: 'Fall', 23: 'Deep Dive', 24: 'Jungle',
	25: 'Mojito', 26: 'Club', 27: 'Christmas', 28: 'Halloween',

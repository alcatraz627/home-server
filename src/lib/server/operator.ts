...
export async function deleteTaskConfig(id: string): Promise<void> {
	const configs = await getTaskConfigs();
	await fs.writeFile(TASKS_FILE, JSON.stringify(configs.filter(c => c.id !== id), null, 2));
}
...
export function getSystemDiskUsage(): { mount: string; total: string; used: string; available: string; usePercent: string }[] {
	try {

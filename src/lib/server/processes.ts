...
export function listProcesses(sortBy: string = 'cpu'): ProcessInfo[] {
	...
	const raw = execSync(
		'ps -eo user,pid,ppid,%cpu,%mem,vsz,rss,state,start,command',
...
export function getProcessDetail(pid: number): ProcessDetail | null {
	...
	try {
		...
		try {
			...
			openFiles = lsofRaw.trim().split('\n').map(l => l.slice(1)).filter(Boolean);
		} catch { /* lsof may fail for privileged processes */ }
		...
		} catch { /* may fail for other users' processes */ }
